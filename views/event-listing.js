...
function events(state, emit) {
  emit('theme', 'gray')
  return state.prismic.getSingle('event_listing', function (err, doc) {
    ...
    const featured = doc.data.featured
      .map(function ({ link }) {
        ...
        return state.prismic.getByUID('event', link.uid, function (err, doc) {
          if (err) return null
          return doc
        })
      })
    ...
    const predicates = [predicate.at('document.type', 'event')]
    ...
    if (typeof events.sort === 'function') {
      events = events
        .map(function (item) {
          if (item) {
        ...
        <div class="u-container u-space2 u-borderB">
          ...
          <div class="u-space2">
            ${grid(
              ...
              events.map(function (doc, index, list) {
                if (!doc) {
                  return grid.cell(
                    { appear: true },
                ...
                return grid.cell(
                  ...
                  card({
                    ...
                    date: {
                      datetime: date,
                      text: doc.data.custom_event_date
