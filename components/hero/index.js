  ...
  createElement (props) {
    ...
    return html`
      <div class="${className('Hero', { 'Hero--small': props.small, 'Hero--pull': props.pull, [`Hero--${props.theme}`]: props.theme })}" id="${this.local.id}">
        ...
        ${props.moving ? html`
          <svg width="46" height="50" viewBox="0 0 46 50" class="Hero-moving Hero-moving--small">
            <path id="${this.local.id}-small-blob" d="${SMALL_BLOB[0]}" fill="currentColor" fill-rule="evenodd"/>
            <animate xlink:href="#${this.local.id}-small-blob" attributeName="d" begin="0s" dur="8s" repeatCount="indefinite" values="${SMALL_BLOB.concat(SMALL_BLOB[0]).join(';')}" />
          </svg>
          <svg width="137" height="138" viewBox="0 0 137 138" class="Hero-moving Hero-moving--medium">
            <path id="${this.local.id}-medium-blob" d="${MEDIUM_BLOB[0]}" fill="currentColor" fill-rule="evenodd"/>
            <animate xlink:href="#${this.local.id}-medium-blob" attributeName="d" begin="0s" dur="10s" repeatCount="indefinite" values="${MEDIUM_BLOB.concat(MEDIUM_BLOB[0]).join(';')}" />
          </svg>
          <svg width="166" height="582" viewBox="0 0 166 582" class="Hero-moving Hero-moving--large">
            <path id="${this.local.id}-large-blob" d="${LARGE_BLOB[0]}" fill="currentColor" fill-rule="evenodd"/>
            <animate xlink:href="#${this.local.id}-large-blob" attributeName="d" begin="0s" dur="6s" repeatCount="indefinite" values="${LARGE_BLOB.concat(LARGE_BLOB[0]).join(';')}" />
          </svg>
        ` : null}
